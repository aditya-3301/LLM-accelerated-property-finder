...
def run_extraction(molecule_input, schema, client, retries=3, wait=10):
    system_prompt = (
        ...
        "- IDENTITY FIELDS: for identity.common_name, identity.cas_number, identity.drugbank_id, "
        "and identity.synonyms, look up the molecule's known registry information. "
        "These are string fields — wrap each as [{\"value\": \"...\", \"confidence\": ..., \"source_type\": \"PubChem\"}]. "
        "For synonyms, provide one candidate dict per name in a list of candidate dicts.\n"

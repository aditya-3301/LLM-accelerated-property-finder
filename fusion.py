...
from drug_descriptors import FIELD_TYPES, source_priority
...
def _effective_confidence(candidate: dict) -> float:
    """
    Blend declared confidence with source-priority score so that
    PubChem/DrugBank values are preferred over literature/other when
    confidence scores are similar.

    Formula: eff = conf * (1 + 0.1 * (priority - 1))
    Priority 5 → ×1.4,  priority 1 → ×1.0.
    The multiplier is deliberately small so that a highly-confident
    literature value can still beat a low-confidence PubChem one.
    """
    conf  = float(candidate.get("confidence", 0))
    prio  = source_priority(candidate.get("source_type", "other"))
    return conf * (1.0 + 0.10 * (prio - 1))
